...
> 完整结果: [results/benchmark-results.md](results/benchmark-results.md)
> 可复现: `cd sync && python benchmark.py 5` (每查询重复 5 次取平均)

#### 4.9.1 存储压缩: MARS3 vs HEAP

同一份数据 (800 万行), 仅引擎不同:

| 引擎 | 压缩 | 总大小 | 压缩率 |
|------|------|--------|--------|
| MARS3 (lz4 level 7) | 列存 + lz4 | 96.3 MB | — |
| HEAP | 行存无压缩 | 573.3 MB | — |
| **节省** | | **477.0 MB** | **83.2%** |

双11月份分区存储是日常月份的 **4.5 倍** (5.4 MB vs 1.2 MB), 直接体现了大促数据膨胀。

#### 4.9.2 查询性能: MARS3 vs HEAP

同一份数据 (800 万行), 每查询执行 5 次取平均:

| 查询类型 | MARS3 (ms) | HEAP (ms) | MARS3 倍速 |
|---------|-----------|----------|-----------|
| Q1 列投影聚合 `SUM(total_amount)` | 327 | 802 | **2.4x** |
| Q2 分组聚合 `GROUP BY status` | 480 | 1240 | **2.6x** |
| Q3 范围过滤+聚合 (11月) | 188 | 525 | **2.8x** |
| Q4 Top-N 排序 `LIMIT 10` | 525 | 1177 | **2.2x** |

**结论**: MARS3 列存在所有 4 类查询上均比 HEAP 快 2.2~2.8 倍。列存只需读取涉及列, 避免全行解码, 配合 lz4 压缩减少 I/O, 优势在聚合和过滤查询上最明显。

#### 4.9.3 分区裁剪

`ods_order_status_events` (70 万行, 13 月分区), EXPLAIN 计划验证分区裁剪生效:

| 场景 | 扫描分区数 | 耗时 (ms) |
|------|-----------|----------|
| 命中单月分区 (11月) | 1 | 27.6 |
| 全表扫描 (13 个分区) | 13 | 38.4 |
| **加速比** | **12/13 分区被跳过** | **1.4x** |

> 注: 20 万订单级别数据量小, 分区裁剪优势不显著。百万级以上规模时, 分区裁剪可跳过 11/12 的物理数据读取, 加速比将显著放大。

#### 4.9.4 物化视图预聚合 vs 实时聚合

同一每日 GMV 指标, 两种计算路径:

| 计算路径 | 耗时 (ms) | 说明 |
|---------|----------|------|
| DWS 物化视图 (REFRESH 预计算) | 4.3 | 直接读结果 |
| DWD 实时聚合 (现场 GROUP BY) | 35.1 | 现场扫描+聚合 |
| **加速比** | **8.1x** | |

**结论**: 物化视图将运行时聚合变为常数级读取, 加速 8 倍。这是数仓分层 DWD→DWS 的核心价值。

#### 4.9.5 ETL 各阶段吞吐

| 阶段 | 行数 | 耗时 (s) | 吞吐 (rows/s) |
|------|------|---------|--------------|
| extract (MySQL→pandas) | 1,583,107 | 16.2 | 97,506 |
| transform (pandas 清洗) | 1,583,107 | 0.4 | 4,048,867 |
| load_ods (mxgate→MARS3) | 1,583,107 | 82.5 | 19,189 |
| load_dim (mxgate→HEAP) | 10,500 | 2.4 | 4,317 |
| dwd_order_fact (INSERT…SELECT) | 200,000 | 2.2 | 92,379 |
| dwd_order_detail (INSERT…SELECT) | 483,200 | 6.1 | 79,148 |
| dwd_order_status_event (INSERT…SELECT) | 699,451 | 2.5 | 283,408 |
| refresh_dws (7 MV REFRESH) | 7 | 3.7 | — |
| **ETL 总耗时** | | **122.8** | |

**瓶颈**: `load_ods` 占总耗时的 67%, 因为通过 `docker exec` stdin 传递 CSV + mxgate 写入 + 构建压缩对比样本 (800 万行 CROSS JOIN)。DWD INSERT…SELECT 纯库内计算吞吐达 28 万 rows/s。
