...
- 客户拿到代码后，一条 `docker-compose up -d && bash init_all.sh` 就能从头跑到尾，零主机环境依赖。
...
| 文件 | 说明 |
|------|------|
| [README.md](../README.md) | 项目说明文档（含部署 FAQ） |
| [init_all.sh](../init_all.sh) | 一键初始化脚本（7 步全链路） |
| [docker-compose.yml](../docker-compose.yml) | 三容器编排 |
| [sync/requirements.txt](../sync/requirements.txt) | Python 依赖锁定版本 |
| [sync/extract.py](../sync/extract.py) | MySQL → DataFrame 抽取 |
...
| [sync/benchmark.py](../sync/benchmark.py) | 性能 Benchmark 脚本 |
